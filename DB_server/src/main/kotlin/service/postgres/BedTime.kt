...
@Serializable
data class BedTime(val id:Int, @Contextual val time: Time, val remindMeToSleep: Boolean, val remindBeforeBad: Boolean)
class BedTimeService(private val connection: Connection){
    companion object {
        ...
        private const val INSERT_BEDTIME = "INSERT INTO bedtime (time, alarm) VALUES (?, ?)"
        ...
        private const val UPDATE_BEDTIME = "UPDATE bedtime SET time = ?, alarm = ? WHERE id = ?"
    ...
    suspend fun save(bedTime: BedTime): Int = withContext(Dispatchers.IO){
        try {
    ...
    suspend fun update(bedTime: BedTime){
        try {
    ...
    suspend fun get(id: Int): BedTime{
